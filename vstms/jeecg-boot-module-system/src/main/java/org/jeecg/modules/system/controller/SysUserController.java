...
public class SysUserController {
    ...
    @RequestMapping(value = "/queryUserRoleName", method = RequestMethod.GET)
    public Result<?> queryUserRoleName(@RequestParam(name = "userid", required = true) String userid) {
        List<SysUserRole> userRole = sysUserRoleService.list(new QueryWrapper<SysUserRole>().lambda().eq(SysUserRole::getUserId, userid));
        if (userRole == null || userRole.size() <= 0) {
            return Result.OK("未找到用户相关角色信息");
        }
        List<String> roleIds = userRole.stream().map(SysUserRole::getRoleId).collect(Collectors.toList());
        List<DictModel> dictModels = sysDictService
                .queryTableDictTextByKeys("sys_role", "role_name", "id", roleIds);
        List<String> roleNames = dictModels.stream().map(DictModel::getText).collect(Collectors.toList());
        return Result.OK(roleNames.stream().collect(Collectors.joining(",")));
    }
