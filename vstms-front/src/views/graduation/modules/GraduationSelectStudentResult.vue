  ...
  <a-card :bordered="false">
    ...
    <div :style="{ marginTop: '15px' }">
      <a-button @click="() => handleSubmit(object)" type="primary" :disabled="disabled" icon="check"
        >完成评选优秀学员结果上传</a-button
      >
      <div class="ant-form-extra" :style="{ fontSize: '12px' }">完成评选优秀学员流程，完成后不可撤销</div>
    </div>
...
import SelectUserModal from './SelectUserModal2.vue'
import { JeecgListMixin } from '@/mixins/JeecgListMixin'
import { putAction } from '@/api/manage'

export default {
  name: 'GraduationSelectStudentResult',
  ...
  data() {
    return {
      ...
      url: {
        list: '/graduation/graduationSelectStudent/list?type=ExcellentList',
        link: '/graduation/graduationSelectStudent/linkResult',
        edit: '/graduation/graduationSelectClass/edit', // 编辑主表信息
        edit2: '/graduation/graduationSelectStudent/edit', // 编辑主表信息
  ...
  methods: {
    ...
    handleDelete(id) {
      const that = this
      let param = { id, status: '0' }
      putAction(that.url.edit2, param).then((res) => {
